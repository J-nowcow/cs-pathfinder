...
구현을 마치고 같은 스크립트를 다시 돌렸더니 결과가 달라졌다.
...
같은 입력, 같은 코드다. **표본 7건짜리 측정은 재현되지 않는다는 것이 실제로 확인됐다.**

다만 **실패 방향이 전부 안전한 쪽이다.** 매칭을 놓쳐 새 노드를 만든 것(NEW)이지 엉뚱한 노드에 붙인 게 아니다. precision은 지켜지고 recall만 떨어졌다. §12가 두 지표를 분리하고 기준을 비대칭으로 잡은 이유가 이것이다.

### 이 수치를 믿으면 안 되는 이유

**표본이 7건이다.** 95% Wilson 신뢰구간이 대략 49~97%다. 실제 성능에 대해 거의 아무것도 확정하지 못한다. 위 재현 실패가 그것을 눈으로 보여준다.
