...
const JUDGED: J[] = [
  ...
  /*
   * 3라운드 (2026-08-10 새벽, 12h 루프 23) — 새로 판정한 7쌍.
   * 0.76+ 이웃 대부분이 "~는 이유는" 문형 오탐이었고 반박을 통과한 것만 남았다.
   */
  [28729, 285, 'prerequisite', '이미지가 무엇인지 알아야 층 캐시로 조금만 받는 것이 읽힌다', 285],
  [28729, 106, 'shares_concept', '변경분만 다시 만들고 전송한다는 같은 원리를 다룬다'],
  [28730, 59, 'shares_concept', '빌드 산출물이 배포 환경을 만나는 지점의 문제를 다룬다'],
  [28730, 194, 'prerequisite', '빌드가 무엇인지 알아야 빌드 통과와 배포 실패의 간극이 읽힌다', 194],
  [28731, 151, 'shares_concept', '요청을 보낼 서버를 고르는 같은 계층을 다룬다'],
  [28731, 284, 'shares_concept', '앞단 중계가 서버 상태를 가리는 같은 구조를 다룬다'],
  [28731, 341, 'shares_concept', '서버의 생사를 밖에서 확인하는 같은 문제를 다룬다'],
]
