...
function main() {
  ...
  /*
   * **앞말을 가리키는 접속어로 시작하는 근거.**
   *
   * 근거 줄은 화면에 **혼자** 뜬다. `그래서 원형 큐를 주로 쓴다`처럼 시작하면
   * "그래서"가 가리킬 앞말이 없어 읽는 사람은 무엇의 결과인지 모른다.
   *
   * 이 모양이 어디서 오는지 알아냈다. 근거를 고를 때 **정답을 진술한 문장을
   * 두고 그 다음 문장을 가져오면** 그 다음 문장이 대개 접속어로 시작한다.
   * 전수로 훑어 392문항을 고쳤고 그 대부분이 이 유형이었다.
   *
   * **이것으로 그 결함을 다 잡지는 못한다.** 접속어 없이도 옆 문장을 가져올
   * 수 있고, 견주는 표에서 반대쪽 행을 가져오는 유형은 여기 안 걸린다.
   * 낱말 겹침으로 넓게 잡아 보려 했는데 34.5%가 걸리고 그 대부분이 멀쩡한
   * 근거였다 — `가용성이 높아진다`를 "잠금을 들고 기다리지 않는다"로 설명하는
   * 것이 좋은 글이지 결함이 아니다. 한국어 바꿔쓰기는 어휘 검사를 이긴다.
   *
   * 그래서 **넓게 잡는 대신 확실한 자리만 막는다.** 지금 0건이고, 늘어난다면
   * 옆 문장을 가져오는 버릇이 돌아온 것이다.
   */
  /*
     * `\b`를 쓰면 안 된다. JS의 단어 경계는 `[A-Za-z0-9_]` 기준이라 한글 뒤에는
     * 경계가 안 생긴다 — `그래서 `에서 "서"와 공백이 둘 다 비단어라 매칭이 실패한다.
     * 처음에 그렇게 써서 검사가 통과만 하고 한 번도 울지 못했다. 일부러 깨뜨려
     * 보지 않았으면 몰랐을 자리다.
     */
    const RATIONALE_OPENER = /^(그래서|그러면|그러니|그렇게|이렇게|그때|따라서|그러므로|대신)(\s|,|$)/
  for (const quiz of NODE_QUIZZES) {
    for (const item of quiz.items) {
      if (!RATIONALE_OPENER.test(item.rationale.trim())) continue
      problems.push({
        where: `${quiz.question} · ${item.stem}`,
        what: '근거가 앞말을 가리키는 접속어로 시작한다. 근거는 혼자 읽히므로 정답을 진술한 문장을 그대로 써라',
      })
    }
  }
