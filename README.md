...
규칙을 적는 것만으로는 부족했다. 예시 20개 중 5개가 도식을 세 문단 뒤에 두고 있었고, 모델은 규칙보다 예시를 따른다. 다섯 개를 다시 배치한 뒤에야 위 숫자가 나왔다. 두 규칙 모두 예시를 상대로 시험이 걸려 있어서 다음에 예시를 늘릴 때 다시 어긋나지 않는다.
...
부팅 시 예시 루트 20개가 자동으로 시드된다. 노드 ID는 질문 텍스트에서 파생하므로 서버를 다시 띄워도 같은 URL이 살아 있다.
...
| `npm run seed` | 주제어 412개와 예시 루트 20개 삽입 |
