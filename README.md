...
가입 없이 바로 쓰면 돼요. **질문 322개**가 올라와 있고(2026년 8월 10일 기준) 매일 하나씩 늘어요.

해설 전문은 [`cs/explanations/`](cs/explanations/)에 **질문 하나에 파일 하나씩** 있어요. 서비스에 안 들어와도 여기서 다 읽으실 수 있어요.
...
| **질문 목록** | `/questions` | 쌓인 질문을 분야 10개로 나눠 훑어요. **태그 16개로 걸러 볼 수도 있고요** |
| **질문 지도** | `/map` | 질문 전부를 별자리처럼 펼쳐 놓고 서로 어떻게 이어지는지 봐요 |
| **내가 판 자리** | `/me` | 며칠에 몇 편 팠는지 잔디로 보고, 다음에 팔 것을 받아요 |
| **용어 사전** | `/glossary` | 해설에 자주 나오는 용어 25개를 한 문장씩 풀어 뒀어요. 본문의 점선 친 용어를 누르면 와요 |
...
본문에 **점선이 밑에 깔린 용어**가 있어요. `스레드`, `TLB` 같은 것들이요. 누르면 [용어 사전](https://cs-pathfinder.vercel.app/glossary)의 그 항목으로 가요. 같은 용어는 본문마다 처음 한 번만 링크가 걸려요.
...
<img src="code/docs/screenshots/questions-tag.png" width="300" align="right" alt="질문 목록에서 동시성 태그를 고른 화면. 태그 칩 줄에서 동시성이 강조돼 있고 아래로 걸러진 질문 24개가 분야별로 나온다" />

질문마다 **분야 하나**와 **태그 몇 개**가 붙어 있어요. 분야는 소속이고 태그는 주제라서 축이 달라요 — "뮤텍스"는 분야로는 운영체제지만 태그로는 `동시성`이에요.

[질문 목록](https://cs-pathfinder.vercel.app/questions)에서 태그를 누르면 그 주제만 모아 봐요. 태그는 16개로 닫혀 있어요. 마음대로 늘리면 `동시성`과 `concurrency`가 따로 생겨서요.

난이도는 `기초 / 심화 / 깊이` 3단이에요. 정답률 실측이 아직 없어서 **판정 기준을 먼저 적고** 그 기준으로 매겼어요. 기준과 한계는 [`code/data/levels.ts`](code/data/levels.ts)에 있어요.
...
**질문은 누가 만드나요?**
AI가 만들어요. 오늘의 질문은 매일 아침 6시에 하나 올라와요. 꼬리질문은 누를 때마다 만들어지고요. 사람이 손으로 쓴 질문 50개가 기준선으로 함께 들어 있어요.
...
**해설이 이상해요.**
틀릴 수 있어요. 얼마나 틀리는지는 아래 "얼마나 믿어도 되나요"에 숫자로 적어 뒀어요. 헤더의 `문의`를 누르면 GitHub 이슈와 메일 주소가 나와요. 어느 쪽이든 좋고, 고치는 법은 [`CONTRIBUTING.md`](CONTRIBUTING.md)에 있어요.

**판 자국은 계정 없이 어떻게 남나요?**
브라우저에만 남겨요. 날짜와 질문 번호만 적고 서버로는 안 보내요. 그래서 기기를 바꾸면 사라져요.

**매일 알아서 받아 볼 수 있나요?**
RSS로요. 읽기 도구에 `https://cs-pathfinder.vercel.app/rss.xml`을 넣으면 매일 아침 올라온 질문이 들어와요. 주소만 넣어도 알아서 찾는 도구가 많고요. 카카오톡 채널은 사업자 등록이 앞에 있어서 아직이에요.
...
2026년 8월 8일에 표본을 떠서 다른 모델(OpenAI Codex)에 넘겨 엄격한 잣대로 대조했더니 **AI가 쓴 20편 중 15편, 사람이 쓴 11편 중 7편**에서 사실 오류가 나왔어요. 대부분 완전한 헛소리가 아니라 **조건을 뺀 단정**이었어요 — "항상", "반드시"가 실제로는 조건부인 경우요. 면접에서 그대로 말하면 되묻히는 문장들이에요.

그래서 **전편을 같은 잣대로 재검했어요.** 2026년 8월 9일까지 모델이 쓴 245편을 4회에 걸쳐 훑어 **오류 59건을 고쳤어요.** 회차마다 오류율이 7%에서 43%까지 갈렸고, 고친 사람이 다음 오류를 만드는 것도 세 번 확인했어요 — 그래서 한 번 훑은 걸로 끝내지 않아요.

잰 방법과 기록은 [`code/docs/audit/`](code/docs/audit/)에 있어요.

**틀린 걸 찾으시면 이슈로 알려 주세요.** 이 저장소가 가장 반기는 기여예요.
