# CS 길라잡이 · cs-pathfinder

**하루에 CS 면접 질문 하나. 궁금한 곳으로 계속 파고들면, 판 만큼 지도가 남는다.**

<https://cs-pathfinder.vercel.app>

<img src="code/docs/screenshots/home.png" width="820" alt="홈. 히어로 아래 오늘의 질문이 주인공으로 서고 그 아래 지난 질문과 게시판이 이어진다" />

## 무엇을 푸는가

카톡방에 매일 CS 면접 질문이 올라온다. 읽을 때는 이해가 되는데, 다음 날이면 어제 것과 오늘 것이 어떻게 이어지는지가 안 남는다. 텍스트가 한 줄로 흐르기 때문이다. "TCP 3-way handshake"를 읽고 "그럼 4-way는?"이 궁금해져도, 그 질문은 어디에도 적히지 않고 사라진다.

이 서비스는 **그 꼬리질문을 클릭 한 번으로 만들고, 만들어진 것들을 그래프로 남긴다.**

## 어떻게 쓰는가

1. 홈에서 **오늘의 질문**을 누른다
2. 해설을 읽으면 아래에 **꼬리질문 5개**가 붙어 있다
3. 궁금한 것을 누르면 그 질문의 해설이 새로 열린다. 원하는 걸 직접 입력해도 된다
4. 깊이 제한이 없다. 계속 파면 계속 갈라진다
5. 다 판 경로는 **링크 하나로 공유**한다. 받은 사람은 같은 자리에서 자기 탐험을 시작할 수 있다

<img src="code/docs/screenshots/reading.png" width="820" alt="읽기 뷰. 4-way handshake가 번호 매긴 순서 도식으로 그려지고 아래로 해설과 꼬리질문이 이어진다" />

가입은 없다. 판 경로는 브라우저에 남고, 링크를 공유하면 그 트리는 누구나 본다.

## 화면 셋

| 화면 | 하는 일 |
|---|---|
| **오늘의 질문** (`/`) | 매일 아침 6시에 올라오는 질문 하나. 지난 질문과 남들이 공유한 트리도 여기 있다 |
| **질문 목록** (`/questions`) | 지금까지 쌓인 질문 249개를 카테고리 10개로 나눠 본다 |
| **지도** (`/map`) | 질문 사이의 의미 관계 330개를 한눈에. 확대할수록 더 많은 질문이 드러난다 |
...
<img src="code/docs/screenshots/diagram.png" width="820" alt="해시 테이블 해설. 완화 방법 두 갈래가 표로 정리되어 있다" />
...
<img src="code/docs/screenshots/shared.png" width="820" alt="공유된 트리. 단면 그래프와 들여쓰기 트리, 추천 버튼, 이어서 파보라는 유도" />

<img src="code/docs/screenshots/mobile.png" width="300" align="right" alt="모바일 홈" />
...
```
cs/            질문 249개 전문. 코드를 안 열어도 무엇이 담겼는지 보인다
code/          앱 전부 — src·data·scripts·tests·설정
README.md      지금 읽는 문서
```

`cs/questions.md`는 발행 워크플로가 매일 다시 쓴다. 손으로 고치지 않는다. `code/data/topic-seeds.ts`의 412개는 "언젠가 질문을 만들 대기열"이지 질문이 아니다.
...
**Docker도 API 키도 필요 없다.** `DATABASE_URL`이 없으면 PGlite(WASM Postgres)로 인메모리에서 돈다. plpgsql까지 그대로 돌아서 실제 의미론으로 테스트할 수 있다. `GOOGLE_GENERATIVE_AI_API_KEY`가 없으면 결정론적 스텁이 대신 답한다 — 해설 첫 문단에 개발용임이 박혀 있어 진짜와 헷갈리지 않는다.

부팅 시 질문 249개(손으로 쓴 30개 + 생성된 219개)와 그 사이의 의미 관계 330개가 자동으로 시드된다. 노드 ID는 질문 텍스트에서 파생하므로 서버를 다시 띄워도 같은 URL이 살아 있다.

---

여기서부터는 만든 사람을 위한 내용이다. 서비스를 쓰는 데는 필요 없다.
...
규칙은 오래 프롬프트에만 있었다. 강제하는 코드는 `scripts/build-generated-nodes.ts`의 `usable()` 하나뿐이었고 그건 코퍼스를 만들 때 한 번 돈 오프라인 배치다. **매일 올라오는 오늘의 질문과 사용자가 파는 모든 질문은 "비었나"만 통과하면 그대로 DB에 들어갔다.**

지금은 `src/lib/llm/content-rules.ts` 한 곳이 규칙을 갖고, 배치와 운영이 같이 부른다.

| | 무엇 | 어긋나면 |
|---|---|---|
| **막는다** | 문단 150자·꼬리질문 35자·꼬리질문 5개·꼬리질문 형식·도식 위치·도식 삼킴 | 지적을 붙여 한 번 다시 부른다 |
...
기준선이 0인 것이 이 표의 요점이다. **기준선을 거는 검사기는 검사기가 틀린 것이다** — 사람이 좋다고 판단한 글을 버리게 만든다.

생성분의 막힐 것이 0인 이유는 이것들이 이미 `usable()`을 통과한 것들이라서다(생존 편향). 그래서 이 숫자로 운영 재시도율을 짐작하면 안 된다. 남은 52건이 전부 문체인 것은 `usable()`이 문체만 한 번도 안 봤기 때문이다.

**문체를 아직 막지 않는 이유.** 다시 부르면 고쳐지는지를 안 쟀다. 번역투는 모델의 습관이라 두 번째도 같을 수 있고, 그러면 확장 한 번이 14초에서 28초가 되면서 얻는 것은 없다. 재시도가 실제로 고치는지 재고 나서 올릴 자리다.

그 과정에서 죽은 검사를 하나 찾았다. `usable()`이 파싱한 **문단**에서 `:::`를 찾고 있었는데, `parseBlocks`는 알아본 울타리도 못 알아본 울타리도 전부 털어내고 문단을 내놓는다. 참이 될 수 없는 조건이라 한 번도 발동한 적이 없다. 진짜 신호는 반대쪽이었다 — **원문에 울타리가 있는데 도식 블록이 안 나오면 그리려던 그림이 통째로 사라진 것**이고, 독자에게는 안 그린 것과 똑같아 보여서 검사하지 않으면 영영 안 보인다.
...
**손으로 쓴 예시는 프롬프트에 안 들어간다.** `code/data/example-nodes.ts`는 DB 시드 전용이다. 그래서 위 숫자는 규칙만으로 나온 결과다.
...
꼬리질문 길이만 반대로 갔다. 4/54가 35자를 넘겼는데 버튼에 그대로 나가는 문장이라 줄이 접힌다. 규칙에 "세어 보고 넘으면 줄인다"와 실제로 길었던 문장의 전후 예를 넣었다. 그 효과는 아직 안 쟀다.

그래도 예시를 같은 규칙에 맞췄다. 사용자가 실제로 읽는 콘텐츠라 생성분과 모양이 다르면 사이트가 두 사람이 쓴 것처럼 읽힌다. 30개 전부에 대해 문단 150자와 도식 위치를 시험으로 묶어두었다.

운영에 던져서도 한 번 확인했다. `POST /api/expand`에 새 질문을 넣으니 14.1초에 `문단 → stack 도식 → 문단 세 개`가 돌아왔고 문단 길이는 72·101·128·91자였다. 합성 측정이 아니라 실제 경로를 그대로 탄 결과다. 다만 한 건이라 비율의 근거는 아니고 경로가 도는지의 확인이다.
...
- **문의 주소가 HTML에 그대로 실린다.** 헤더의 `mailto:`가 수집 봇에 긁힌다. GitHub 이슈보다 문턱이 낮은 쪽을 골랐고, 스팸이 실제로 오면 폼으로 바꾼다
- **무료 한도가 마르면 오늘의 질문 발행이 사실상 멈춘다.** 사슬을 계측해서 원인을 갈랐다.
  ...
  사슬은 더 이상 병목이 아니다. 남은 원인은 Gemma의 응답 시간이 회차마다 크게 튀는 것이다. 같은 날 발행을 세 번 돌리니 40.0초에 한 번 성공하고, 한 번은 과부하 통지로, 한 번은 40초 제한에 걸려 실패했다.

  그 과부하 통지에서 버그를 하나 잡았다. `This model is currently experiencing high demand.`가 `fatal`로 분류돼 폴백 사슬을 통째로 건너뛰고 있었다. 낱말 목록에 걸리는 것이 하나도 없어서였다. 과부하는 저쪽 사정이라 다른 모델에서는 되는데 시도조차 안 했다. `transient`로 고쳤다.

  대응은 두 겹이다. 워크플로가 30초 간격으로 **세 번까지 다시 던진다** — 응답 시간이 회차마다 갈리니 한 번 실패했다고 그날 질문이 없어질 이유가 없다(실제로 08-09는 세 번째 시도에서 통과했다). 하루 하나는 DB가 보장하므로 재시도가 중복 발행을 만들지 않는다. 그리고 한도가 살아 있는 시간대에 며칠치를 미리 뽑아둘 수 있다 — 미래 발행분은 목록에도 직접 URL에도 안 나오므로 하루 하나라는 약속은 지켜진다.
