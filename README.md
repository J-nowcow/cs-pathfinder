...
- **무료 한도가 마르면 오늘의 질문 발행이 사실상 멈춘다.** 상위 모델이 전부 한도에 걸리면 사슬 끝의 Gemma가 답하는 자리가 되는데, 여기서 걸리는 시간이 회차마다 크게 흔들린다. `generateDailyRoot` 단독 호출은 25.9초에 성공했지만(3.5-flash quota 7.0초 → 3.6-flash quota 13.7초 → 성공), 같은 설정으로 실제 발행은 시도 제한을 150초까지 늘려도 여덟 번 연속 시간 초과였다. **어느 쪽이 지배적인 원인인지는 아직 못 갈랐다** — 프롬프트 크기일 수도, quota 실패 지연의 변동일 수도, 시드 주제어에 따른 차이일 수도 있다. 확장(`/api/expand`)은 같은 날 14초에 정상 응답했다.

  당장의 대응은 한도가 있는 시간대에 며칠치를 미리 뽑아두는 것이다. 미래 발행분은 목록에도 직접 URL에도 노출되지 않으므로 미리 뽑아도 하루 하나라는 약속은 지켜진다. 다음에 손댈 때는 원인부터 갈라야 한다 — 같은 시드 주제어로 여러 번 재보고, 발행 프롬프트를 줄인 판과 나란히 재면 프롬프트 크기 탓인지 아닌지가 한 번에 갈린다. 원인을 모른 채 프롬프트를 줄이면 품질만 깎고 문제는 그대로일 수 있다
- 주제어 시드 412개 = 13개월치
