...
- **무료 한도가 마르면 오늘의 질문 발행이 사실상 멈춘다.** 사슬을 계측해서 원인을 갈랐다.
  ...
  | 구간 | 걸린 시간 |
  |---|---|
  | 3.5-flash → quota 실패 | 9.0초 |
  | 3.6-flash → quota 실패 | 15.9초 (누적) |
  | Gemma가 답하는 자리 | **10초일 때도, 120초를 넘길 때도 있다** |

  프롬프트 크기 탓이 아니다. 같은 프롬프트·같은 시드 주제어로 `generateDailyRoot`를 다섯 번 부르니 전부 24.8~26.6초에 성공했다. 실패는 Gemma 응답 시간이 길게 튀는 회차에서만 난다.

  값이 큰 것은 quota 실패조차 9초, 16초씩 든다는 점이다. 발행 라우트 예산이 60초인데 사슬을 지나는 데만 16초를 쓰고 나면 답하는 모델에게 44초밖에 못 준다. 확장(`/api/expand`)은 3.6-flash 버킷이 살아 있어 같은 날 14초에 정상 응답했다.
