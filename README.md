...
가입 없이 바로 쓰면 돼요. **질문 312개**가 올라와 있고(2026년 8월 8일 기준) 매일 하나씩 늘어요.
...
**해설 초안은 AI가 써요.** 그래서 정확도를 짐작으로 말하지 않고 재고 있어요.

2026년 8월 8일에 표본을 떠서 다른 모델(OpenAI Codex)에 넘겨 엄격한 잣대로 대조했어요.
결과는 이랬어요.
...
대부분은 완전한 헛소리가 아니라 **조건을 뺀 단정**이었어요. "항상", "반드시", "~할 수 없다" 같은 말이 실제로는 조건부인 경우요. 면접에서 그대로 말하면 되묻히는 문장들이에요.

지금 전부를 같은 잣대로 훑으면서 고치고 있어요. 잰 방법과 진행 상황은
[`code/docs/audit/2026-08-08-factcheck-strict.md`](code/docs/audit/2026-08-08-factcheck-strict.md)에 있어요.
