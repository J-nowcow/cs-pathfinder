...
가입 없이 바로 쓰면 돼요. 지금까지 쌓인 질문은 251개예요.
...
| **질문 목록** | `/questions` | 쌓인 질문 251개를 분야 10개로 나눠 훑어요 |
...
답 바로 뒤에는 **그림**이 와요. 순서가 있으면 번호가 붙은 단계로, 층이 쌓였으면 위아래로 쌓인 칸으로, 둘을 견주면 표로 나와요. 줄글로 읽고 머리로 다시 그리는 수고를 덜어 드리려고요.
...
> 지도가 두 개예요. 헤더의 `지도`는 서비스 전체 251개, 질문 화면 아래쪽의 `지도`는 내가 판 것만이에요.
