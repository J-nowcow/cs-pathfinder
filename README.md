...
- **무료 한도가 마르면 오늘의 질문 발행이 사실상 멈춘다.** 사슬을 계측해서 원인을 갈랐다.
  ...
  당장의 대응은 한도가 있는 시간대에 며칠치를 미리 뽑아두는 것이다. 미래 발행분은 목록에도 직접 URL에도 노출되지 않으므로 미리 뽑아도 하루 하나라는 약속은 지켜진다. 근본 해결은 Gemma에 기대지 않는 것이다. 사슬 끝을 응답 시간이 안정적인 모델로 바꾸거나, 발행만은 한도가 살아 있는 시간대에 미리 돌리는 편이 낫다. 프롬프트를 줄이는 것은 답이 아니다 — 위 실측이 프롬프트 크기를 이미 배제했다
- 주제어 시드 412개 = 13개월치
