...
- **무료 한도가 마르면 오늘의 질문 발행이 사실상 멈춘다.** 상위 모델이 전부 한도에 걸리면 사슬 끝의 Gemma가 답하는데, 발행 프롬프트가 커서 Gemma가 이것을 제 시간에 끝내지 못한다. 짧은 프롬프트는 18~25초에 답하고 `generateDailyRoot` 단독 호출도 한 번은 25.9초에 성공했지만, 실제 발행은 시도 제한을 150초까지 늘려도 계속 시간 초과였다. 확장(`/api/expand`)은 같은 날 14초에 정상 응답했다 — 그쪽은 프롬프트가 작고 3.6-flash 버킷이 살아 있었다.

  당장의 대응은 한도가 있는 시간대에 며칠치를 미리 뽑아두는 것이다. 미래 발행분은 목록에도 직접 URL에도 노출되지 않으므로 미리 뽑아도 하루 하나라는 약속은 지켜진다. 근본 해결은 발행 프롬프트를 Gemma가 감당할 크기로 줄이거나 사슬 끝에 다른 모델을 두는 것인데, 둘 다 품질 실측이 먼저 필요하다
- 주제어 시드 412개 = 13개월치
