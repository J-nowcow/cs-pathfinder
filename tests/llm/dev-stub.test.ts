...
const gate = (parent: string | null, input: string) =>
  runGate({ parentQuestion: parent, rawInput: input, call: stubCaller })

describe('stubCaller — 정규화 게이트', () => {
  ...
  it('normalizes the same input to the same sentence so the cache can hit', async () => {
    const a = await gate(null, '인덱스가 왜 안 타나요?')
    const b = await gate(null, '  인덱스가   왜 안 타나요  ')

    expect(a.relevant && b.relevant).toBe(true)
