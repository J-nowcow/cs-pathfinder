 ...
 * 카테고리 원의 반지름.
 *
 * **자랄 크기에 맞춰 미리 크게 잡는다.** 개수에 따라 늘리면 질문이 하나
 * 추가될 때마다 모든 카테고리가 바깥으로 밀려서, 같은 질문이 어제와 다른
 * 자리에 놓인다. 이 배치가 지키려던 것이 바로 그것이라 그럴 수 없다.
 *
 * 카테고리당 40개(전체 400개)까지 아무것도 겹치지 않는 값을 격자 탐색으로
 * 찾았다. 그 규모에서 같은 카테고리 안 최소 간격 250, 카테고리 간 385이고
 * 카드 폭이 210이라 여유가 남는다.
 *
 * 지금처럼 적을 때는 성기게 보인다. 열었을 때 전체에 맞춰 배율이 잡히고
 * 카테고리 이름은 배율과 무관하게 읽히므로 문제가 되지 않는다.
 */
const RING = 4800
 ...
 * 나선 한 바퀴에 놓는 질문 수.
 *
 * 한 바퀴에 적게 놓으면 바퀴 수가 늘고 덩어리 반경이 커진다. 반경이 커지는
 * 것이 카테고리끼리 부딪히는 원인이라 넉넉히 놓는다.
 */
const PER_TURN = 11
